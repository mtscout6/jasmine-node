...
function serveStaticFiles(request, response){
  paperboy
    ...
    .after(function(statCode) {
      logger.info('Ended Request: ' + request.url + ' [' + statCode + ']');
    })
    .error(function(statCode, msg) {
      response.writeHead(statCode, {'Content-Type': 'text/plain'});
      response.end('Error ' + statCode);
      logger.info('ERROR: ' + statCode + ' ' + request.url);
    })
    .otherwise(function(err){
      response.writeHead(404, {'Content-Type': 'text/plain'});
      response.end('Error 404: File not found');
      logger.info('404: ' + request.url);
    });
};

function resolveCssRequests(request, response){
  logger.info('LESS REQUEST: ' + request.url);
...
function handleWebSocketConnection(socket){
  logger.info('----Handling web socket connection');
